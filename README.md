...
## DCASE 2026 Challenge Task 2 and DCASE {2025, 2024, 2023} Challenge Task 2 Baseline Auto Encoder: dcase2023\_task2\_baseline\_ae

This is an autoencoder-based baseline for the [DCASE2026 Challenge Task 2 (DCASE2026T2)](https://dcase.community/challenge2026/), the [DCASE2025 Challenge Task 2 (DCASE2025T2)](https://dcase.community/challenge2025/), the [DCASE2024 Challenge Task 2 (DCASE2024T2)](https://dcase.community/challenge2024/) and the [DCASE2023 Challenge Task 2 (DCASE2023T2)](https://dcase.community/challenge2023/).

This source code is an example implementation of the baseline Auto Encoder of DCASE2026T2: Noise-aware Unsupervised Anomalous Sound Detection for Machine Condition Monitoring, and 
DCASE{2025, 2024, 2023}T2: First-Shot Unsupervised Anomalous Sound Detection for Machine Condition Monitoring.
...
- Data folder structure is updated to support DCASE2026T2, DCASE2025T2, DCASE2024T2 and DCASE2023T2 data sets.
...
The difference between the 2026T2 baseline and {2025, 2024, 2023}T2 baselines is that 2026T2 takes a two-channel input containing near-mic (in the left channel) and far-mic (in the right channel). The signal in the left channel is equivalent to the previous mono.
...
- 02a\_test\_2026t2.sh (Use MSE as a score function for the Simple Autoencoder mode)
  ...
  <!-- - "Evaluation" mode:
    - This script makes a CSV file for each section, including the anomaly scores for each wav file in the directories `data/dcase2026t2/eval_data/raw/<machine_type>/test/`. (These directories will be made available with the "evaluation dataset".)
...
- 03\_summarize\_results.sh
  - This script summarizes results into a CSV file.

C.f., for DCASE2025T2, DCASE2024T2 and DCASE2023T2  see [README\_legacy](README_legacy.md).

### Dataset audio format

Please note that the audio format differs depending on the challenge year:

- DCASE2026 Task 2 datasets are provided in stereo (2-channel) format.
- DCASE2020 Task 2 ~ DCASE2025 Task 2 datasets are provided in monaural (single-channel) format.

In the baseline, only channel 0 (the left channel) of the stereo audio data is used.
...
We will launch the datasets in three stages. Therefore, please download the datasets in each stage:
  ...
  + For DCASE 2023 Challenge Task 2
	(C.f., for DCASE2023T2, see [README\_legacy](README_legacy.md))
    + "Development Dataset"
...
### 3. Unzip the downloaded files and make the directory structure as follows:
  ...
  + dcase2023\_task2\_baseline\_ae
    + data/dcase2026t2/dev\_data/raw/
      + \<machine\_type0\_of\_development\_dataset\>/
...
Note that if values are specified in the command line option, they will overwrite the parameter settings in `baseline.yaml`.

### 4.1. Enable Auto-download dataset

If you haven't yet downloaded the dataset yourself or run the download script (e.g., `data_download_2026dev.sh`), you may want to use the auto-download.
To enable the auto-downloading, set the parameter `--is_auto_download` (default: `False`) to `True` in `baseline.yaml`. If `--is_auto_download` is `True`, then auto-download is executed.

### 5. Run the training script (for the development dataset)

Run the training script `01_train_2026t2.sh`. Use the option -d for the development dataset `data/dcase2026t2/dev_data/<machine_type>/raw/train/`.

```dotnetcli
# Train the system using the DCASE2026 Task 2 Datasets
$ 01_train_2026t2.sh -d
```
The two operating modes of this baseline implementation, the simple Autoencoder and the Selective Mahalanobis AE modes, share the common training process. By running the script `01_train_2026t2.sh`, all model parameters for the simple Autoencoder and the selective Mahalanobis AE will be trained simultaneously.
After the parameter update of the Autoencoder at the last epoch specified by either the yaml file or the command line option, the covariance matrices for the Mahalanobis distance calculation will be set.

### 6. Run the test script (for the development dataset)

### 6.1. Testing with the Simple Autoencoder mode
Run the test script `02a_test_2026t2.sh`. Use the option `-d` for the development dataset `data/dcase2026t2/dev_data/<machine_type>/raw/test/`.

```dotnetcli
# Test the Simple Autoencoder mode using the DCASE2026 Task 2 Datasets
$ 02a_test_2026t2.sh -d
```

The `02a_test_2026t2.sh` options are the same as those for `01_train_2026t2.sh`. `02a_test_2026t2.sh` calculates an anomaly score for each wav file in the directories `data/dcase2026t2/dev_data/raw/<machine_type>/test/` or `data/dcase2026t2/dev_data/raw/<machine_type>/source_test/` and `data/dcase2026t2/dev_data/raw/<machine_type>/target_test/`.
...
Run the test script `02b_test_2026t2.sh`. Use the option `-d` for the development dataset `data/dcase2026t2/dev_data/<machine_type>/raw/test/`.

```dotnetcli
# Test the Selective Mahalanobis mode using DCASE2026 Task 2 Datasets
$ 02b_test_2026t2.sh -d
```

The `02b_test_2026t2.sh` options are the same as those for `01_train_2026t2.sh`. `02b_test_2026t2.sh` calculates an anomaly score for each wav file in the directories `data/dcase2026t2/dev_data/raw/<machine_type>/test/` or `data/dcase2026t2/dev_data/raw/<machine_type>/source_test/` and `data/dcase2026t2/dev_data/raw/<machine_type>/target_test/`.
A CSV file for each section, including the anomaly scores, will be stored in the directory `results/`. If the mode is "development", the script also outputs another CSV file, including AUC, pAUC, precision, recall, and F1-score for each section.
...
Each anomaly score corresponds to a wav file in the directories `data/dcase2026t2/dev_data/<machine_type>/test/`.
...
After the evaluation dataset for the test is launched, download and unzip it. Move it to `data/dcase2026t2/eval_data/raw/<machine_type>/test/`. Run the test script `02a_test_2026t2.sh` with the option `-e`.

```dotnetcli
$ 02a_test_2026t2.sh -e
```

Anomaly scores are calculated using the evaluation dataset, i.e., `data/dcase2026t2/eval_data/raw/<machine_type>/test/`. The anomaly scores are stored in CSV files in the `results/` directory. You can submit the CSV files for the challenge. From the submitted CSV files, we will calculate AUC, pAUC, and your ranking.
...
After launching the evaluation dataset for testing, download and unzip it. Move it to `data/dcase2026t2/eval_data/raw/<machine_type>/test/`. Run the `02b_test_2026t2.sh` test script with the option `-e`.

```dotnetcli
$ 02b_test_2026t2.sh -e
```
...
After executing `02a_test_2026t2.sh`, `02b_test_2026t2.sh`, or both. Run the summarize script `03_summarize_results.sh` with the option `DCASE2026T2 -d`.

```dotnetcli
# Summarize development dataset 2026
$ 03_summarize_results.sh DCASE2026T2 -d
```

After the summary, the results are exported to `results/dev_data/baseline/summarize/DCASE2026T2` in CSV format.

If you want to change the summarize results directory or the export directory, edit `03_summarize_results.sh`.

After executing `02a_test_2026t2.sh`, `02b_test_2026t2.sh`, or both. Run the summarize script `03_summarize_results.sh` with the option `DCASE2026T2 -d` or `DCASE2026T2 -e`.
...
After the summary, the results are exported in CSV format to `results/dev_data/baseline/summarize/DCASE2026T2` or `results/eval_data/baseline/summarize/DCASE2026T2`.

If you want to change the summarize results directory or the export directory, edit `03_summarize_results.sh`.

## Legacy support

This version takes the legacy datasets provided in DCASE2020 task2, DCASE2021 task2, DCASE2022 task2, DCASE2023 task2, DCASE2024 task2 and DCASE2025 Task2 dataset for inputs.
...
- Provides support for the development datasets used in DCASE2026T2.
...
The following code was used to calculate the official score. Among these is the evaluation dataset ground truth.

- [dcase2025_task2_evaluator](https://github.com/nttcslab/dcase2025_task2_evaluator)

### Ground truth for evaluation datasets in this repository

This repository contains the ground truth CSV for the evaluation data. This CSV is used to rename evaluation datasets.
You can calculate AUC and other scores if you add ground truth to the evaluation dataset's file name. *Usually, the rename function is executed along with [download script](#description) and [auto download function](#41-enable-auto-download-dataset).
...
+ Tomoya Nishida, Noboru Harada, Daisuke Niizumi, Keisuke Imoto, Kota Dohi, Harsh Purohit, Takashi Endo, and Yohei Kawaguchi. Description and discussion on DCASE 2026 challenge task 2: Noise-aware Unsupervised Anomalous Sound Detection for Machine Condition Monitoring. In arXiv e-prints: xxxx.xxxx, 2026. [URL](https://arxiv.org/pdf/)
