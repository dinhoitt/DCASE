...
from datasets.dcase_dcase202x_t2_waveform_loader import DCASE202XT2WaveformLoader

class DCASE202XT2(object):
    def __init__(self, args):
        if args.model == "MED-ASD":
            self.setup_waveform_med_asd(args)
            return
    ...
    def setup_waveform_med_asd(self, args):
        self.width = args.med_audio_samples
        self.height = 1
        self.channel = 2
        self.input_dim = self.width * self.height * self.channel
        shuffle = args.shuffle
        batch_sampler = None
        batch_size = args.med_batch_size
        print("input dim: %d" % (self.input_dim))
        ...
        train_data = DCASE202XT2WaveformLoader(
            data_path,
            dataset_name=dataset_name,
            section_keyword=section_keyword,
            machine_type=machine_type,
            ...
            use_id=args.use_ids,
            is_auto_download=args.is_auto_download,
            sample_rate=args.med_sample_rate,
            audio_samples=args.med_audio_samples,
        )
        train_index, valid_index = train_test_split(range(len(train_data)), test_size=args.validation_split)
        self.train_dataset = Subset(train_data, train_index)
        self.train_loader = torch.utils.data.DataLoader(
        ...
        if args.train_only:
            return
        for id in self.section_id_list:
            _test_loader = DCASE202XT2WaveformLoader(
                data_path,
                dataset_name=dataset_name,
                section_keyword=section_keyword,
                machine_type=machine_type,
